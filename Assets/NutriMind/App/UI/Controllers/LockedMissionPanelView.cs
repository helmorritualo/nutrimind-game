...
namespace NutriMind.App.UI
{
    ...
    public sealed class LockedMissionPanelView : IAppScreenView
    {
        ...
        {
            [MissionLockReason.TeacherRestricted] = new("The Grand Holiday Chronicle", "The restored pages must be bound into the Grand Holiday Chronicle, but each district wants its part placed first. The Pathfinder must create one fair, coherent record for the whole community.", "Locked by Teacher", "This mission is currently locked by your Teacher. Your existing progress is safe.", "Prerequisite complete — no additional missions required.", "Waiting for classroom release", "Downloaded • Available offline", "Back to Missions", "ds-icon--lock", "ds-icon--arrow-left", true),
            [MissionLockReason.PrerequisiteRequired] = new("The Newsroom of True Pages", "Festival reports have been mixed with rumors. The town newsroom cannot publish until the Pathfinder separates evidence from opinion and rebuilds the report.", "Prerequisite Required", "Complete Mission 3 before starting this mission.", "Requires: The Hall of Speaking Sounds (Mission 3)", "Published to your classroom", "Downloaded • Available offline", "Continue Required Mission", "ds-icon--lock", "ds-icon--chevron-right", false),
        ...
        public LockedMissionPanelView(VisualElement root)
        {
            ...
            CacheElements();
            RegisterCallbacks();
            SetContext(new LockedMissionPreviewContext(NutriMindSubject.LiteraQuest, NutriMindTerm.Term1, 5, "The Grand Holiday Chronicle", MissionLockReason.TeacherRestricted, "Prerequisite complete — no additional missions required."));
