...
- Treat design-only, presentation-only, and Penpot-only UI work as visual/layout tasks: UXML/USS, Penpot boards, and shared UI assets only—App panel controllers stay presentation-only with static preview data (no API, SQLite, auth, sync, mission loading, or production routing) unless explicitly asked; Quiz Portal previews must show canonical/server-provided scores as-is (never recalculate) and must not invent selected answers, correct answers, explanations, or per-question points; treat announcement bodies as plain text only (never execute HTML or rich-text markup from announcement content); wire App panels together only after the screen set is designed; when migrating a panel into AppShell, strip duplicated global shell chrome and keep the screen content-only so it works both embedded and as a standalone UIDocument preview; BootstrapPanel is a standalone pre-route screen (never hosted in AppShell or AppShellContentPreview, no bottom nav).
- Match supplied design references closely—exact padding, margins, type sizes, logo/text placement, and panel centering; when asked for layout-only, copy structure and spacing first and ignore reference artwork until imagery is requested; for large refinement passes, inspect references and existing UXML/USS first, plan the highest-impact fixes, and wait for approval before rewriting broadly when Plan Mode is requested; when explicitly asked to copy mission terrain into `SCN_G5_LQ_T1_M01_TheFestivalStorybookRescue`, base it on the Polytope Studio demo terrain with lightly flat ground and follow the user-supplied three-area festival layout rather than inventing a new terrain style.
- Prefer Full HD (1920×1080) landscape framing for static UI layout tests, Penpot boards, and gameplay HUD preview, while keeping App UITK layouts responsive across Android screen sizes.
- For the gameplay prototype, build screen-space HUD and mobile touch controls with UI Toolkit only—no screen-space uGUI Canvas; keep HUD controllers presentation-only with preview data and UI intent events until player or mission integration is explicitly requested.
...
- Omit Demo Login and other non-reference chrome unless explicitly requested; on Subject Selection and Term Selection screens, omit Profile/Settings/Logout chrome and the Quiz Portal card (Quiz Portal belongs on Home); AppShell More opens Leaderboard (Profile stays via the profile button, not More); while Certificates is open keep Rewards bottom-nav active; while Announcements or Leaderboard is open keep More active.
...
- On Profile, do not expose a full raw LRN (use a masked preview); style Sign Out as a clear red/destructive action; on Leaderboard, use privacy-safe display aliases only and prominently identify the current learner.
...
- Environment kits: Polytope Studio lives under `Assets/Polytope Studio/` (demo `Lowpoly_Demos/Environment_Free`, terrain `PT_Terrain_mat`); Medieval Village MegaKit URP remaps live under `Assets/Materials/Fixes`; keep Polytope building materials on URP Lit with atlas textures—Built-in Amplify shaders render magenta/pink under URP.
- The project targets Unity 6 (`6000.3.x` per `ProjectSettings/ProjectVersion.txt`).
- Static App UITK screen and shared-component previews (BootstrapPanel as standalone pre-route, Login, Home, Subject/Term/Mission Selection, Locked Mission, Profile, Settings, Progress, QuizListPanel, QuizDetailPanel, QuizAttemptPanel, QuizResultPanel, QuizHistoryPanel, MissionDetailPanel, RewardsPanel, CertificatesPanel, AnnouncementsPanel, LeaderboardPanel, AppShell, DataStatePanel, ConfirmDialog, SystemDialog, LoadingOverlay, OfflineSyncBanner) remain wired through `Assets/Scenes/SampleScene.unity` (`AppUiPreview`) for dual-host checks; dedicated runtime App scenes are `SCN_App_Bootstrap`, `SCN_App_Authentication`, `SCN_App_Main`, and `SCN_App_QuizPortal` (Build Settings 0–3) with `PFB_AppLifetime`; prefer Unity MCP Inspector wiring when the Editor is stable, otherwise give step-by-step manual wiring instead of repeatedly retrying heavy MCP calls.
- AppShell content migration uses `IAppScreenView`, `AppScreenContent.uss`, and `AppShellContentPreviewController` to host content-only route screens (embedded with `app-screen-content--embedded` or standalone UIDocument); Home, Subject Selection, Term Selection, Mission Selection, Locked Mission, Profile, Settings, Progress, QuizListPanel (Quiz Portal list), QuizDetailPanel (Quiz Portal detail), QuizAttemptPanel (Quiz Portal attempt), QuizResultPanel (Quiz Portal result), QuizHistoryPanel (Quiz Portal history), MissionDetailPanel, RewardsPanel, CertificatesPanel, AnnouncementsPanel, and LeaderboardPanel are all migrated, each with a reusable `<Panel>View`, a standalone controller, and an AppShell factory case—AppShell-embedded is the primary authenticated-route preview and standalone SampleScene objects are secondary dual-host checks; secondary routes keep the originating bottom-nav item active (Certificates→Rewards; Announcements/Leaderboard→More); shared scroll rules in `AppScreenContent.uss` size scroll content to its children so screens scroll instead of being squeezed and clipped.
- Gameplay student HUD prototype lives under `Assets/NutriMind/Gameplay/UI/UITK/HUD/` (`GameplayStudentHud.uxml`, view, preview controller, joystick/look manipulators) with dedicated `PS_GameplayStudentHud` PanelSettings (1920×1080 reference, match 0.5); this is an intentional UITK override of older uGUI HUD docs and is not yet wired to player control or mission runtime.
- App UITK iteration commonly uses the Singularity Group Hot Reload package; pause Hot Reload and free RAM before Unity MCP `ManageGameObject`/scene wiring; use Unity MCP sparingly (prefer `ReadConsole`, `ValidateScript`, and narrow `RunCommand`; avoid hierarchy dumps, broad object searches, list-property overwrites, and forced reimports) so the Editor does not hang in an infinite reload; mock API fixture loads (`Resources.Load` under `NutriMindMock`) must run on the main thread—preload at gateway construction and never load after `ConfigureAwait(false)`/background delays.
